...
function renderSources(report) {
  const sources = Array.isArray(report.sources) ? report.sources.filter(Boolean) : [];
  if (!sources.length) return '';
  const chips = sources.map((source) => `<li>${esc(source)}</li>`).join('');
  return `
    <section class="report-sources" aria-label="sources">
      <p class="report-sources-label">Sources</p>
      <ul class="report-source-list">${chips}</ul>
    </section>
  `;
}
...
export function renderReport(report) {
  ...
  const blocks = sortedBlocks
    .map((block, index) => {
      ...
      return `<article id="${esc(block.sectionId)}" class="content-section" data-section-index="${index + 1}">${inner}</article>`;
    })
    .join('');
  ...
  return `
    <div class="report-shell">
      ...
      <button id="mobile-sidebar-backdrop" class="mobile-sidebar-backdrop" type="button" aria-label="메뉴 닫기"></button>
      ...
      <aside id="sidebar" class="report-sidebar -translate-x-full">
        <header class="report-sidebar-header">
          ...
          <small>${esc(report.date ?? '')} · ${esc(report.theme ?? 'general')}</small>
          ${sources}
        </header>
        ${nav}
      </aside>
      ...
      <main class="report-main">
        <header class="report-header report-block">
          <p class="report-kicker">Briefing</p>
          <h2>${esc(report.title)}</h2>
          <p>${esc(report.description ?? '')}</p>
          ${sources}
        </header>
        <section class="report-content report-content-wrapper">${blocks}</section>
      </main>
    </div>
  `;
}
