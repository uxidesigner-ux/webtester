...
document.addEventListener('DOMContentLoaded', () => {
      ...
      if (mb && sb) mb.addEventListener('click', () => sb.classList.toggle('-translate-x-full'));
      // ScrollSpy
      const navLinks = document.querySelectorAll('#main-nav a');
      const sections = document.querySelectorAll('.content-section');
      ...
      function updateNav() {
        ...
        while (--i && window.scrollY + 120 < sections[i].offsetTop) {}
        navLinks.forEach(l => {
          l.classList.remove('bg-red-500', 'text-white');
          l.classList.add('text-gray-600');
        });
        if (i >= 0 && navLinks[i]) {
          navLinks[i].classList.remove('text-gray-600');
          navLinks[i].classList.add('bg-red-500', 'text-white');
        }
      }
      navLinks.forEach(link => {
        link.addEventListener('click', function(e) {
          ...
          if (window.innerWidth < 768 && sb) sb.classList.add('-translate-x-full');
        });
      });
